...
\begin{equation}
  ...
  \label{eq:kQRR}
\end{equation}
where $p^\ddagger(\chip)$ is the distribution of path spreads at the dividing surface and $\kappa(\chip)$ is a class-conditional transmission coefficient. When $\kappa$ is taken independent of $\chip$, Eq.~\eqref{eq:kQRR} reduces identically to ordinary centroid QTST: $\kQRR=\kcTST$. QRR decomposes a known estimator rather than claiming a more accurate thermal rate. It reports which imaginary-time geometries carry the flux, how that mix depends on isotope and barrier, and whether path spread couples to a solvent coordinate.
...
\begin{equation}
  ...
  \bigr\rangle.
\end{equation}
A Bennett--Chandler transmission coefficient $\kappa(\chip)$ may be attached to each class.\cite{bennett1977,chandler1978,craig2005} Integrating over $\chip$ produces Eq.~\eqref{eq:kQRR}. When $\kappa$ does not depend on $\chip$, the integral collapses to $\kQRR=\kappa\,\kQTST$, and the choice $\kappa=1$ gives
\begin{equation}
  \kQRR(\kappa=1)\equiv\kcTST.
  \label{eq:identity}
\end{equation}
The Eckart calculations below evaluate this identity by computing $\kcTST$ from umbrella PIMC and comparing it to the exact scattering rate. Two further restrictions apply. A Wigner series in $\hbar^2$ is a tunneling correction to a parabolic barrier,\cite{wigner1932,wigner1938} not a recrossing factor; the two must not be conflated. A $\chip$-dependent $\kappa$ is not computed: for highly delocalized paths the Wigner series is not a controlled approximation, and a class-conditional RPMD flux-side correlation would be the consistent dynamical correction.\cite{craig2005} Constructing a $\kappa(\chip)$ from the centroid potential of mean force would double-count the barrier and cannot repair the QTST overestimate documented below.
